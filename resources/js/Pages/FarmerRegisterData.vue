...
const form = useForm({
    ...
    location: props.homestead?.location || props.prefill?.location || null,
});

function moveMarker(event) {
    markers.value = []
    markers.value.push(event.latlng)
    form.location = latlng_to_gjson_point(markers[0]) || null
}

function latlng_to_gjson_point(latlng) {
    if (latlng != undefined) {
        return  { type: "Feature", geometry: { type: "Point", coordinates: [latlng.lat, latlng.lng] } }
    }
    return null
}

function gjson_point_to_latlng(gjson) {
    if (gjson) {
        return {lng: gjson.geometry.coordinates[0], lat: gjson.geometry.coordinates[1] }
    }
    return null
}
...
const zoom = ref(8);
const submit = () => {
    form.location = latlng_to_gjson_point(markers[0]);
    if (props.farmstead) {
...
onMounted(() => {
    markers.value[0] = gjson_point_to_latlng(form.location) || undefined,
    center.value = form.location || [52.05140211668126, 6.3209299468511535]
})
        ...
        <form @submit.prevent="submit">
            ...
            <div class="mt-4 flex flex-row">
                ...
                <div>
                    ...
                    <div class="flex justify-center w-full my-2">
                        <Checkbox id="show_email" name="show_email" class="w-7 h-7"/>
                    </div>
                    <InputError class="mt-2" :message="form.errors.email" />
                </div>
            </div>
            <div class="mt-4 flex flex-row">
                ...
                <div>
                    ...
                    <div class="flex justify-center w-full my-2">
                        <Checkbox id="show_number" name="show_number" class="w-7 h-7"/>
                    </div>
                    <InputError class="mt-2" :message="form.errors.phone_number" />
                </div>
            </div>
